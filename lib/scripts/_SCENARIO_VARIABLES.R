...
if (exists("SYSTEM_COMMAND_ARGS")) {
  ...
  if (length(SYSTEM_COMMAND_ARGS) >= 2) SCENARIO_YEAR <- as.integer(SYSTEM_COMMAND_ARGS[2]) else SCENARIO_YEAR <- as.integer(BASE_SCENARIO_BASE_YEAR)
  if (length(SYSTEM_COMMAND_ARGS) >= 3) SCENARIO_RUN_FIRMSYN <- as.logical(SYSTEM_COMMAND_ARGS[3]) else SCENARIO_RUN_FIRMSYN <- TRUE
  if (length(SYSTEM_COMMAND_ARGS) >= 4) SCENARIO_RUN_CVTM    <- as.logical(SYSTEM_COMMAND_ARGS[4]) else SCENARIO_RUN_CVTM <- TRUE
  if (length(SYSTEM_COMMAND_ARGS) >= 5) SCENARIO_RUN_TT      <- as.logical(SYSTEM_COMMAND_ARGS[5]) else SCENARIO_RUN_TT <- TRUE
  if (length(SYSTEM_COMMAND_ARGS) >= 6) SCENARIO_RUN_DB      <- as.logical(SYSTEM_COMMAND_ARGS[6]) else SCENARIO_RUN_DB <- TRUE
  if (length(SYSTEM_COMMAND_ARGS) >= 7) SCENARIO_REFERENCE_NAME <- SYSTEM_COMMAND_ARGS[7] else SCENARIO_REFERENCE_NAME <- ifelse(SCENARIO_NAME == BASE_SCENARIO_BASE_NAME, "Validation", BASE_SCENARIO_BASE_NAME)
  if (length(SYSTEM_COMMAND_ARGS) >= 8) SCENARIO_REFERENCE_YEAR <- as.integer(SYSTEM_COMMAND_ARGS[8]) else SCENARIO_REFERENCE_YEAR <- as.integer(BASE_SCENARIO_BASE_YEAR)
