...
dat_ref <- cv_trips[TripID == 1, .(Tours = .N), keyby = .(TourType, Vehicle)]
dat <- rbind(dat_scen[, Source := SCENARIO_NAME], dat_ref[, Source := SCENARIO_REFERENCE_NAME])
...
# Organize
dat_scen <- cv_trips[TripID == MinTripID, .(Tours = .N), keyby = .(Vehicle, Hour = floor(MAMArrive/60))]
dat_ref <- ref_cv_trips[TripID == MinTripID, .(Tours = .N), keyby = .(Vehicle, Hour = floor(MAMArrive/60))]
