...
```{r Chart_tt_val_vmt_tab}
# Organize

dat <- cal_results$calibration_targets_tt_sim$tt_build$vmt_est_fhwa[,.(Vehicle, Total_VMT = TotalVMT, Observed_CSVM_VMT = CSV_Int_VMT)]

dat[tmh_vtods[,.(Model_VMT = sum(VMT)), keyby = .(Vehicle)],
    Model_VMT := i.Model_VMT, on = "Vehicle"]

dat[, Difference := Model_VMT - Observed_CSVM_VMT]

dat <- add_totals(dat, rowtotal = FALSE)                  

dat[, Ratio := Model_VMT/Observed_CSVM_VMT]

# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "tt_build_val_vmt_tab.csv"))
assign("db_tab_tt_build_val_vmt_tab", dat, envir = db_inputs)
...
> Total_VMT is estimated commercial vehicle VMT based on FHWA reported average daily VMT by vehicle type for the Chicago IL-IN urban area in 2017, factored up to the 21 county model region with light vehicle VMT allocated to commercial based on the proportion from the SEMCOG region and factored to average weekday. Observed_CSVM_VMT is the proportion of the VMT that is internal to internal and is from an industry covered by the model (i.e, excluding freight model covered industries, manufacturing and transportation and warehousing).
...
dat[, Difference := Model - Observed]
dat[, Ratio := Model/Observed]
...
```{r Chart_tt_val_District_origins}
# Organize
dat_obs <- cal_results$calibration_targets_tt_sim$tt_build$districtod_all[, .(TargetTrips = sum(n)), keyby = .(OriginDistrict = ODistrict)]
dat_obs[, Target := TargetTrips/sum(TargetTrips)]
dat_mod <- TripTable[, .(ModelTrips = sum(trips, na.rm = TRUE)), keyby = .(OriginDistrict = OSummaryGeog)]
dat_mod[, Model := ModelTrips/sum(ModelTrips)]

dat <- merge(dat_obs, dat_mod, by= "OriginDistrict", all = TRUE)
...
dat[, OriginDistrict := factor(OriginDistrict, levels = levels(TAZ_System$DistrictName))]
setorder(dat, OriginDistrict)
...
# Table
kable(dat[,.(OriginDistrict, Target, Model, Difference, Ratio)], escape=FALSE, digits = 3) %>%
  kable_styling(fixed_thead = TRUE, bootstrap_options = c("condensed")) 
...
```{r Chart_tt_val_District_dest}
# Organize
dat_obs <- cal_results$calibration_targets_tt_sim$tt_build$districtod_all[, .(TargetTrips = sum(n)), keyby = .(DestinationDistrict = DDistrict)]
dat_obs[, Target := TargetTrips/sum(TargetTrips)]
dat_mod <- TripTable[, .(ModelTrips = sum(trips, na.rm = TRUE)), keyby = .(DestinationDistrict = DSummaryGeog)]
dat_mod[, Model := ModelTrips/sum(ModelTrips)]

dat <- merge(dat_obs, dat_mod, by= "DestinationDistrict", all = TRUE)
dat[is.na(Target), Target := 0]
dat[is.na(Model), Model := 0]
...
dat[, DestinationDistrict := factor(DestinationDistrict, levels = levels(TAZ_System$DistrictName))]
setorder(dat, DestinationDistrict)

# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "tt_build_district_dest_all.csv"))
assign("db_tab_tt_build_district_dest_all", dat, envir = db_inputs)

# Table
kable(dat[,.(DestinationDistrict, Target, Model, Difference, Ratio)], escape=FALSE, digits = 3) %>%
  kable_styling(fixed_thead = TRUE, bootstrap_options = c("condensed")) 
...
```{r Chart_tt_val_District_origins_light}
# Organize
dat_obs <- cal_results$calibration_targets_tt_sim$tt_build$districtod_light[, .(TargetTrips = sum(n)), keyby = .(OriginDistrict = ODistrict)]
dat_obs[, Target := TargetTrips/sum(TargetTrips)]
dat_mod <- TripTable[Vehicle == "Light", .(ModelTrips = sum(trips, na.rm = TRUE)), keyby = .(OriginDistrict = OSummaryGeog)]
dat_mod[, Model := ModelTrips/sum(ModelTrips)]
...
# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "tt_build_district_origins_light.csv"))
assign("db_tab_tt_build_district_origins_light", dat, envir = db_inputs)
...
```{r Chart_tt_val_District_origins_medium}
# Organize
dat_obs <- cal_results$calibration_targets_tt_sim$tt_build$districtod_medium[, .(TargetTrips = sum(n)), keyby = .(OriginDistrict = ODistrict)]
dat_obs[, Target := TargetTrips/sum(TargetTrips)]
dat_mod <- TripTable[Vehicle == "Medium", .(ModelTrips = sum(trips, na.rm = TRUE)), keyby = .(OriginDistrict = OSummaryGeog)]
dat_mod[, Model := ModelTrips/sum(ModelTrips)]
...
# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "tt_build_district_origins_medium.csv"))
assign("db_tab_tt_build_district_origins_medium", dat, envir = db_inputs)
...
### Percent Trips by District OD: Model

```{r Chart_Percent_Trips_By_District_OD_val_mod}
# Organize
dat_mod <- TripTable[, .(ModelTrips = sum(trips, na.rm = TRUE)), by = .(OriginDistrict = OSummaryGeog, DestinationDistrict = DSummaryGeog)]
dat_mod[, Model := ModelTrips/sum(ModelTrips)]
...
# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "tt_build_district_od_val_mod.csv"))
assign("db_tab_tt_build_district_od_val_mod", dat, envir = db_inputs)
...
### Percent Trips by District OD: Observed

```{r Chart_Percent_Trips_By_District_OD_obs}
# Organize
dat_obs <- cal_results$calibration_targets_tt_sim$tt_build$districtod_all[, .(TargetTrips = sum(n)), keyby = .(OriginDistrict = ODistrict, DestinationDistrict = DDistrict)]
dat_obs[, Target := TargetTrips/sum(TargetTrips)]
dat_obs[, OriginDistrict := factor(OriginDistrict, levels = levels(TAZ_System$DistrictName))]
dat_obs[, DestinationDistrict := factor(DestinationDistrict, levels = levels(TAZ_System$DistrictName))]
...
# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "tt_build_district_od_val_obs.csv"))
assign("db_tab_tt_build_district_od_val_obs", dat, envir = db_inputs)
...
### Percent Trips by District OD: Difference (Model - Observed)

```{r Chart_Percent_Trips_By_District_OD_diff}
# Organize
dat_diff <- merge(dat_obs, dat_mod, by = c("OriginDistrict", "DestinationDistrict"))
...
# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "tt_build_district_od_val_diff.csv"))
assign("db_tab_tt_build_district_od_val_diff", dat, envir = db_inputs)
...
# Organize
dat_mod <- TripTable[, .(ModelTrips = sum(trips)), keyby = .(OriginDistrict = ODistrictName, Distance = round(dist,-1))]
dat_mod[is.na(ModelTrips), ModelTrips := 0]
dat_mod[, Model := ModelTrips/sum(ModelTrips), by = OriginDistrict]

dat_obs <- cal_results$calibration_targets_tt_sim$tt_build$tripdistancedist_district[, .(TargetTrips = sum(n)), keyby = .(OriginDistrict = tripOrigin_District, Distance = round(Trip_dist,-1))]
dat_obs[is.na(TargetTrips), TargetTrips := 0]
dat_obs[, Target := TargetTrips/sum(TargetTrips), by = OriginDistrict]

dat <- merge(dat_obs, dat_mod, by = c("Distance", "OriginDistrict"), all = TRUE)
dat[is.na(Target), Target := 0]
dat[is.na(Model), Model := 0]
...
dat_mod <- add_totals(dcast.data.table(dat,
                                  DistanceLabel ~ OriginDistrict,
                                   fun.aggregate = sum,
                                   value.var = "Model"),
                      rowtotal = FALSE)

# Save
fwrite(dat_mod, file = file.path(SCENARIO_OUTPUT_PATH, "tt_build_district_distance_val_mod.csv"))
assign("db_tab_tt_build_district_distance_val_mod", dat_mod, envir = db_inputs)
...
# Save
fwrite(dat_obs, file = file.path(SCENARIO_OUTPUT_PATH, "tt_build_district_distance_val_obs.csv"))
assign("db_tab_tt_build_district_distance_val_obs", dat_obs, envir = db_inputs)
...
# Save
fwrite(dat_obs, file = file.path(SCENARIO_OUTPUT_PATH, "tt_build_district_distance_val_diff.csv"))
assign("db_tab_tt_build_district_distance_val_diff", dat_obs, envir = db_inputs)
...
# Organize
dat <- melt.data.table(dat[, .(Distance, OriginDistrict, Model, Target)], id.vars = c("OriginDistrict", "Distance"), variable.name = "Source", value.name = "PctTrips")
...
# Plot
bar_plotter(dat[OriginDistrict == "Chicago"], xvar = "Distance", yvar = "PctTrips",  fill = "Source", position = "dodge", ylabel = "Percentage of Trips", png_name = "p_tt_build_tripdistancedist_chicago_val.png", xrotate = TRUE)
...
# Plot
bar_plotter(dat[OriginDistrict == "COOK, IL (Outside Chicago)"], xvar = "Distance", yvar = "PctTrips",  fill = "Source", position = "dodge", ylabel = "Percentage of Trips", png_name = "p_tt_build_tripdistancedist_cook_val.png", xrotate = TRUE)
```

### Percent Trips by Distance for Rest of CMAP MPO Area (Outside Cook)

```{r Chart_tt_trip_dist_cmap_Val}
dat <- dat[!OriginDistrict %in% c("Chicago", "COOK, IL (Outside Chicago)", "Non-CMAP Part of Model Regio"),
...
### Tours With Stops in Same TAZ

```{r Chart_tt_tour_repeatstops_Val}
# Organize

# for this summary comparing the final set of tour stops
# use the cv_trips table as the TripTable is too aggregated
dat_mod_reps <- cv_trips[Activity != "Return", .(NStops = .N), keyby = .(TourID, DTAZ)]
dat_mod_reps <- dat_mod_reps[NStops > 1, .(RepeatZones = .N, Visits = sum(NStops)), keyby = TourID]
dat_mod_tours <- cv_trips[Activity != "Return",.(NStops = .N), keyby = .(TourID)]
dat_mod_tours[dat_mod_reps, c("RepeatZones", "Visits") := .(i.RepeatZones, i.Visits), on = "TourID"]
dat_mod_tours[is.na(RepeatZones), RepeatZones := 0]
dat_mod_tours[is.na(Visits), Visits := 0]
dat_mod <- dat_mod_tours[,.(ModelTours = .N), keyby = RepeatZones]
dat_mod[, Model := ModelTours/sum(ModelTours)]

dat_obs <- cal_results$calibration_targets_tt_sim$tt_build$gps_tour_repzones_unique[,.(RepeatZones, TargetTours = Tours, Target = Target_amongAllTours)]
dat_obs <- rbind(data.table(RepeatZones = 0, TargetTours = 0, Target = 1 - sum(dat_obs$Target)),
                 dat_obs)
dat_obs[RepeatZones == 0, TargetTours := sum(dat_obs[RepeatZones > 0]$TargetTours)/sum(dat_obs[RepeatZones > 0]$Target) * Target]

dat <- merge(dat_obs, dat_mod, by= "RepeatZones", all = TRUE)
dat[is.na(Target), Target := 0]
dat[is.na(Model), Model := 0]

dat <- melt.data.table(dat[, .(RepeatZones, Model, Target)], id.vars = c("RepeatZones"), variable.name = "Source", value.name = "PctTours")
dat[, RepeatZones := factor(RepeatZones)]

# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "tt_build_tourrepeatstops_val.csv"))
assign("db_tab_tt_build_tourrepeatstops_val", dat, envir = db_inputs)

# Plot
bar_plotter(dat, xvar = "RepeatZones", yvar = "PctTours",  fill = "Source", position = "dodge", xlabel = "Number of Repeated Stops (Same Zone) on the Tour", ylabel = "Percentage of Tours", png_name = "p_tt_build_tourrepeatstops_val.png")
...
# for this summary comparing the final set of trip start times
# use the cv_trips table as the TripTable is too aggregated
dat_mod <- cv_trips[, .(ModelTrips = .N), keyby = .(TOD, OriginDistrict = Region.Origin)]
dat_mod[, Model := ModelTrips/sum(ModelTrips), keyby = OriginDistrict]
dat_obs <- cal_results$calibration_targets_tt_sim$tt_build$tripstarttimedist_district[,.(Target = sum(Target)), keyby = .(StopTime = floor(StopDepartHour), OriginDistrict = tripOrigin_district)]
...
dat_obs <- dat_obs[,.(Target = sum(Target)), keyby = .(TOD, OriginDistrict)]

dat <- merge(dat_obs, dat_mod[,.(OriginDistrict, TOD, Model)], by = c("TOD", "OriginDistrict"), all = TRUE)
dat[is.na(Target), Target := 0]
dat[is.na(Model), Model := 0]

dat <- melt.data.table(dat, id.vars = c("OriginDistrict", "TOD"), variable.name = "Source", value.name = "PctTrips")
dat[tod_labels, Hours := i.Hours, on = "TOD"]
dat[, TOD_Label := paste(TOD, Hours)]
...
# Plot
bar_plotter(dat[OriginDistrict == "Chicago"], xvar = "TOD_Label", yvar = "PctTrips",  fill = "Source", position = "dodge", xlabel = "Trip Start Time Period", ylabel = "Percentage of Trips", xrotate = TRUE, png_name = "p_tt_build_tripstarttime_tod_chicago_val.png")
...
# Plot
bar_plotter(dat[OriginDistrict == "COOK, IL (Outside Chicago)"], xvar = "TOD_Label", yvar = "PctTrips",  fill = "Source", position = "dodge", xlabel = "Trip Start Time Period", ylabel = "Percentage of Trips", xrotate = TRUE, png_name = "p_tt_build_tripstarttime_tod_cook_val.png")
