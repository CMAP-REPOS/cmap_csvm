...
# Organize
dat <- cal_results$cv_sim_scheduledstops$submodel_extra$submodel_comparison_stop_dist[, .(Activity, Target = round(Target,2), Model = round(Model,2), Ratio = round(Model/Target,2))]

# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "cv_sim_scheduledstops_base_stop_activity_tab.csv"), row.names = FALSE)
assign("db_tab_cv_sim_scheduledstops_base_stop_activity_tab", dat, envir = db_inputs)
...
# Organize
dat <- cal_results$cv_sim_scheduledstops$submodel_extra$submodel_comparison_stop_dist_ind[Activity == "Goods", .(Industry = EmpCatGroupedName, Target, Model)]

dat <- melt.data.table(dat, id.vars = c("Industry"), variable.name = "Source", value.name = "Distance")

# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "cv_sim_scheduledstops_base_stop_industry_goods.csv"))
assign("db_tab_cv_sim_scheduledstops_base_stop_industry_goods", dat, envir = db_inputs)

# Plot
bar_plotter(dat, xvar = "Industry", yvar = "Distance",  fill = "Source", position = "dodge", coord_flip = TRUE, ylabel = "Base to Stop Distance", png_name = "p_cv_sim_scheduledstops_base_stop_industry_goods.png")
...
# Organize
dat <- cal_results$cv_sim_scheduledstops$submodel_extra$submodel_comparison_stop_dist_ind[Activity == "Service", .(Industry = EmpCatGroupedName, Target, Model)]

dat <- melt.data.table(dat, id.vars = c("Industry"), variable.name = "Source", value.name = "Distance")

# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "cv_sim_scheduledstops_base_stop_industry_service.csv"))
assign("db_tab_cv_sim_scheduledstops_base_stop_industry_service", dat, envir = db_inputs)

# Plot
bar_plotter(dat, xvar = "Industry", yvar = "Distance",  fill = "Source", position = "dodge", coord_flip = TRUE, ylabel = "Base to Stop Distance", png_name = "p_cv_sim_scheduledstops_base_stop_industry_service.png")
...
# Plot
bar_plotter(dat[Activity == "Goods"], xvar = "stop_duration", yvar = "PctStops",  fill = "Source", position = "dodge", ylabel = "Percentage of Stops", xlabel = "Stop Duration (Minutes)", xrotate = TRUE, png_name = "p_cv_sim_stopduration_activity_goods.png")
...
# Plot
bar_plotter(dat[Activity == "Service"], xvar = "stop_duration", yvar = "PctStops",  fill = "Source", position = "dodge", ylabel = "Percentage of Stops", xlabel = "Stop Duration (Minutes)", xrotate = TRUE, png_name = "p_cv_sim_stopduration_activity_service.png")
...
### cv_sim_tours: Definitions

```{r Chart_cv_sim_tours_Descr}
# Show table
tour_types_table <- data.table(Tour_Type = c("BBM", "BBS", "BNM", "BNS", "NBM", "NBS", "NNM", "NNS"),
                         Description = c("Tour starts at truck base and ends at truck base, with multuple stops",
                                         "Tour starts at truck base and ends at truck base, with single stop",
                                         "Tour starts at truck base and ends not at truck base, with multuple stops",
                                         "Tour starts at truck base and ends not at truck base, with single stop",
                                         "Tour starts not at truck base and ends at truck base, with multuple stops",
                                         "Tour starts not at truck base and ends at truck base, with single stop",
                                         "Tour starts not at truck base and ends not at truck base, with multuple stops",
                                         "Tour starts not at truck base and ends not at truck base, with single stop"))

kable(tour_types_table, escape=FALSE) %>%
  kable_styling(fixed_thead = TRUE, bootstrap_options = c("striped"))

```

### cv_sim_tours: Single vs Multiple Stop

```{r Chart_cv_sim_tours_single_multiple}
# Organize
dat <- cal_results$cv_sim_tours$submodel_comparison[, .(ModelTours = sum(ModelTours), TargetTours = sum(TargetTours)), keyby = .(NumStops = SingleMultiple, Vehicle)]

dat[, c("Model", "Target") := .(ModelTours/sum(ModelTours), TargetTours/sum(TargetTours)), by = Vehicle]

dat <- melt.data.table(dat[,.(NumStops, Vehicle, Model, Target)], id.vars = c("NumStops", "Vehicle"), variable.name = "Source", value.name = "PctTours")

# Save
fwrite(dat, file = file.path(SCENARIO_OUTPUT_PATH, "cv_sim_tours_single_multi_vehicle.csv"))
assign("db_tab_cv_sim_tours_single_multi_vehicle", dat, envir = db_inputs)

# Plot
bar_plotter(dat[NumStops == "Single"], xvar = "Vehicle", yvar = "PctTours",  fill = "Source", position = "dodge", coord_flip = TRUE, ylabel = "Percentage of Tours that are Single Stop", png_name = "p_cv_sim_tours_single_multi_vehicle.png")
