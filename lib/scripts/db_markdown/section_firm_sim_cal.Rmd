...
# Organize
dat <- cal_results$firm_sim_taz_land_use$submodel_comparison[Category %in% c("NEmp_Total"), .(Model = sum(Model), Target = sum(Target)), keyby = .(County = SummaryGeog)]

dat <- melt.data.table(dat, id.vars = "County", variable.name = "Source", value.name = "Employment")
dat[, County := factor(County, levels = levels(TAZ_System$DistrictName))]
...
# Organize
dat <- cal_results$firm_sim_taz_land_use$submodel_comparison[Category %in% c("HH"), .(Model = sum(Model), Target = sum(Target)), keyby = .(County = SummaryGeog)]
