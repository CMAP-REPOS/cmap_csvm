...
# How much do the skims changes between runs? Do we have a single consistent set that can be used for
# Target development and calibration?

# Reading minial set of processed data rather than every estimation dataset -- which are used?

# Reading model objects from estimated versions, not in system data, in case they get adjusted in cal.
...
stops_taz[TAZSocioEconomics, c("HH", "POP") := .(i.HH, i.POP), on = "TAZ"]
...
cv_intermediate_data[UEmpCats, IndustryCat := i.EmpCatGroupedName, on = "EmpCatName"]
...
### cv_sim_externalstations ----------------------------------------------------------------------------------

# Calibration of the cv_sim_externalstations model:

# This model allocated an external station based on the skimmed route between internal and buffer zones
# The number of trips at each external is based on counts
# For heavy trucks, long distance model accounts for some, while remainder are from short haul IE/EI/EE trips from the CVTM
# For medium trucks, all counts are assumed to be from short haul travel, modeled as IE/EI/EE trips from CVTM
# For light trucks, number is unknowm as counts include passenger, estimate reasonableness -- should be somewhat similar in distribution to medium

# Heavy truck targets for cv portion at externals are generated by the ld targets script
# Easiest way to compare and summarize trips by externals is at the trip table step, do LD and CV 
# Here should just confirm that the allocation by OD pair of the externals matches that in the skims

# Develop a unique list of O-Ext-D for IE/EI trips and O-Ext1-Ext2-D for EE trips from skims
external_xi <- skims_buffer[SKIMTYPE == "Buffer to SEMCOG",.(OTAZ, DTAZ, TOD, EXT)]
external_ix <- skims_buffer[SKIMTYPE == "SEMCOG to Buffer",.(OTAZ, DTAZ, TOD, EXT)]
external_xx <- skims_buffer[SKIMTYPE == "Buffer traversing SEMCOG",.(OTAZ, DTAZ, TOD, EXT1, EXT2)]

# Add the targets to the list
model_step_targets_cv_sim[["cv_sim_externalstations"]] <- list(external_xi = external_xi,
                                                               external_ix = external_ix,
                                                               external_xx = external_xx)
